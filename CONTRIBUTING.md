...
Thanks for considering a contribution to `shared-web`.
...
A local Redis is required (used for caching and sessions) - `redis-server` on `localhost:6379`
with no auth is sufficient for tests.
